...
In this article we're going to take a deep insight to some of the internal of the system and
to a specific feature called "Alpha Network Compiler", which has been present for a while in the code base but only 
recently has started to being used actively.

You might know that the rules evaluation is based on a data structure called Rete ("network" in Latin) which is a  
tree comprised of different possible kind of nodes. Here's some examples of nodes:
...
The Alpha Network Compiler is an optimization that is effective when the Rete is an "Alpha Network" which is a shortcut to say it's a Rete
consisting only of Object Type Nodes, Alpha Nodes and Rule Terminal Nodes. In this articles, for the sake of simplicity there won't be any 
references to other kinds of node.

When dealing with Drools rules it's useful to see the relationship between the nodes in a graphical way, so here's an example of a very simple Rete.
...
We have a yellow node called "entry-point", which is where the evaluation of the rules start. 
Every object (also called "fact") inserted inside Drools' working memory is evaluated against each node in order, if the evaluation succeed it's propagated to the next one until it reach the Rule Terminal Node.

Even though in this article we're going to create a simple rete for evaluation, I'll also try to put the references of the original source code of Drools, so that you can check how the real production code works. 

# What is an Object Type Node? 

Think of an [Object Type Node](https://github.com/lucamolteni/drools/blob/54c3bf1a0b1297b3d1594cf3229d9e6218dd1ebc/drools-core/src/main/java/org/drools/core/reteoo/ObjectTypeNode.java#L54
)  as a Java `instanceof` operation. If an object matches the type defined in the OTN it will enter the Rete from that node. 
There's one OTN for each Java Type defined in each pattern. If two rules has two different patterns on the same object type 
...
In the diagram shown before, we have a Pattern with three constraints, the blue nodes.
...
consequences of the rules, but we skip the explanation of the Agenda as it's not useful to the purpose of this article.

# Alpha Node Sharing

Let us see the first optimization of the PHREAK network: Alpha Node Sharing.
When we have more than one pattern in different rules, as we said before, each constraint inside a pattern will generate a single alpha node, but when the
first part of the pattern is identical only a single Alpha Nodes will be created. 
...
We might think of a common [interface](https://github.com/lucamolteni/drools/blob/9e49bb752401671ffd854421f9feefd915f4a591/drools-core/src/main/java/org/drools/core/reteoo/ObjectSink.java#L33) for Drools nodes
...
Here's an example on how we could create a simple ObjectTypeNode (the actual code is much more complicated as there's some caching of these types: 
...
And here's an example on how we could create a simple Alpha Nodes 
...
After being evaluated in the Object Type Node, the OTN will call assertObject on the first child, which will evaluate
...
When there are multiple non related Alpha Nodes, each Alpha Node will be checked against the input object such as (see [Drools'](https://github.com/kiegroup/drools/blob/54c3bf1a0b1297b3d1594cf3229d9e6218dd1ebc/drools-core/src/main/java/org/drools/core/reteoo/CompositeObjectSinkAdapter.java#L619))
This is not efficient as we need to check an if with dynamic dispatch against every single Alpha Nodes. 
A common project might have thousands, if not tens of thousand Alpha Nodes!
This approach is simple enough and works well for demonstration purposes, but is terribly inefficient. A Java Predicate can express
every possible boolean condition, but it's not the fastest way to evaluate what in the Java world could be a simple if.
Using a Java predicate will mean using dynamic dispatch as the JVM doesn't know the concrete implementation of the `DroolsNode` interface.

Here's the "brute force" evaluation of all the constraints. 
...
When a field is hashable, for example when dealing with String and Integers, after a certain amount of Alpha Nodes the system will keep an internal map of the Alpha Nodes using the actual value as the key.
...
When we build the rete, we could do something among this line: 
...
What I wanted to show you is just one of the optimisation that is in the code bases. 
There are many more, the purpose of which is to collapse the search space reducing the number of evaluations.
...
As you can see there are no alpha nodes in this propagateAssertObject method. 
The constraints are unwrapped from the alpha node and set as a field of the class.

At least the order of the ifs will be determined by the Alpha Node Sharing optimization we saw before, so it's faster than evaluating the ifs in a brute force order.
For example with these rules, with one identical constraint as before:
...
```
By inlining the strings as keyword inside a switch case we leverage Java internal dispatch to get even faster results.
...
Here's an example of such table, for more information check the [documentation](https://docs.jboss.org/drools/release/7.53.0.Final/drools-docs/html_single/index.html#dmn-decision-tables-con_dmn-models).
...
We don't consider only the compiled Alpha Network as an evaluation optimisation, with alpha node sharing, inlining and range indexing,
but we think in the future it could become also a "lean" representation of an Alpha Network without using the full rete generate by the executable model.
...
at compile time, so that when instantiating the compiled Alpha Network we don't need the full rete. 
Currently this is not doable, if you check the `setNetworkNodeReference` that unwraps the constraint in fields you can see it takes the node of the Rete as an input,
but this is totally avoidable.
...
optimization used while generating the Compiled Alpha Network. After that we saw an experimental usage of the Alpha Network to evaluate in an efficient way DMN Decision Table.
